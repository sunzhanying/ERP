...
public class DepotAction extends BaseAction<DepotModel> {
    ...
    public void create() {
        ...
        try {
            ...
            //更新jsh_userbusiness 将登录用户添加depot仓库管理权限
            System.out.println(depot);
            System.out.println(depot.getId());
            String currentUserId = model.getUBKeyId();
            if(currentUserId != null && !"".equals(currentUserId)) {
            	String type = "UserDepot";
            	List<UserBusiness> currentUserBusinessList = userBusinessService.findUserBusinessListByUserId(Long.valueOf(currentUserId),type);
            	if(currentUserBusinessList != null && currentUserBusinessList.size() > 0){
            		UserBusiness ub = currentUserBusinessList.get(0);
            		String targetValue = ub.getValue() + "[" + depot.getId() + "]";
            		ub.setValue(targetValue);
            		userBusinessService.update(ub);
            	}
